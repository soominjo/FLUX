    ...
    <div className="flex items-center justify-between p-3 rounded-lg border bg-zinc-900/50 border-lime-400/20">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-[10px] text-zinc-500">Wants to be your Workout Buddy</div>
        </div>
      </div>
...
export function BuddyRequests() {
  ...
  // Filter only BUDDY type requests
  const buddyRequests = (requests || [])
    .filter((rel: Relationship) => rel.type === 'BUDDY' && rel.id)
    .map((rel: Relationship) => ({
      ...rel,
      id: rel.id as string, // Type assertion after filtering
    }))
  ...
  const handleRespond = (connectionId: string, action: 'ACCEPT' | 'REJECT') => {
    respond(
      ...
      {
        onSuccess: () =>
          toast.success(action === 'ACCEPT' ? 'Buddy request accepted!' : 'Buddy request declined'),
  ...
  if (buddyRequests.length === 0) return null
